...
function Dashboard() {
  ...
  const handleCheckIn = async () => {
    ...
    const cairoOffset = 2 * 60 * 60 * 1000; // 2 hours in milliseconds for Cairo (UTC+2)
    
    // Adjust to Cairo time
    const cairoTime = new Date(utcTime.getTime() + cairoOffset);
    
    // Convert to UTC ISO string for storage
    const checkInTime = new Date(cairoTime.getTime() - cairoOffset).toISOString();
    const displayTime = timeFormatter.format(cairoTime); // For display
  ...
  const handleCheckOut = async () => {
    ...
    const displayTime = timeFormatter.format(new Date(checkOutTime));
